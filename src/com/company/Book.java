...
public class Book {
    ...
    public static void addOrder(String orderID, String side, int price, int size) {
        ...
        if(side.equals("buy")) {
            ...
            buyPriceSize[price] += size;
            if(price > maxBuyPriceInCents) maxBuyPriceInCents = price;
            if(price < minBuyPriceInCents) minBuyPriceInCents = price;
        } else {
            sellHM.put(newOrder.orderID, newOrder);
            sellPriceSize[price] += size;
            if(price > maxSellPriceInCents) maxSellPriceInCents = price;
            if(price < minSellPriceInCents) minSellPriceInCents = price;
        }
    }
    ...
    // complete order will be on sideOfOrder against book, for us, so us buying, hit sells
